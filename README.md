...
## Передача интегратору Langflow (handoff)

Этот репозиторий в текущем виде — **клиент к Langflow по HTTP**. Дальнейшая реализация логики потоков должна делаться на стороне Langflow (структура графов, промпты, провайдер LLM, маршрутизация ответов).

### Что уже есть в приложении

- Полный UI для запуска 3-шагового конвейера (`1. форма`, `2. заполнение`, `3. проверка`).
- Сбор входного текста из папок с документами.
- Вызов `POST /api/v1/run/{flow_id}` для каждого шага.
- Автопоиск UUID потоков по именам `NormDocs — 1.*`, `NormDocs — 2.*`, `NormDocs — 3.*`.
- Поддержка явных UUID через переменные окружения.

### Что нужно сделать интегратору

1. Поднять Langflow и создать API-ключ.
2. Создать/настроить 3 рабочих потока (`NormDocs — 1...`, `NormDocs — 2...`, `NormDocs — 3...`).
3. Убедиться, что каждый поток стабильно возвращает текст в `Chat Output` для API-вызова.
4. Проверить вызовы из GUI по шагам (по одному), затем прогон «Выполнить все три».
5. При необходимости зафиксировать UUID потоков через `LANGFLOW_FLOW_FORM`, `LANGFLOW_FLOW_FILL`, `LANGFLOW_FLOW_VERIFY`.

### Минимальный smoke-check перед сдачей

- Шаг 1 возвращает непустую структуру отчета.
- Шаг 2 возвращает заполненный черновик по входным данным.
- Шаг 3 возвращает проверку/замечания по нормативке.
- В журнале GUI нет `403/401`, таймаутов и пустых `outputs`.
